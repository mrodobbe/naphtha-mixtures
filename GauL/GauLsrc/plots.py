...
def performance_plot(outputs, predictions, test, prop, folder="newPredictions", fold=0, model="ANN"):
    if prop == "h" or prop == "Enthalpy":
        ...
        prop = "Entropy"
        metric = "J mol$^{-1}$ K$^{-1}$"
    elif prop == "bp" or prop == "Boiling Point":
        prop = "Boiling Point"
        metric = "K"
    elif prop == "tc" or prop == "Critical Temperature":
        prop = "Critical Temperature"
        metric = "K"
    elif prop == "sg" or prop == "Density":
        prop = "Entropy"
        metric = "kg m$^{-3}$"
    elif prop == "vap" or prop == "Vapor Pressure":
        prop = "Vapor Pressure"
        metric = "log(mmHg)"
...
def output_plot(molecules, outputs, prop, name="output_plot", folder="newPredictions"):
    if prop == "h":
        ...
        y_label = "Experimental Boiling Point [K]"
        if min(outputs) < 250:
            outputs += 273.15
    elif prop == "tc":
        y_label = "Experimental Critical Temperature [K]"
        if min(outputs) < 250:
            outputs += 273.15
    elif prop == "sg":
        y_label = "Experimental Density [kg m$^{-3}$]"
        if min(outputs) < 1:
            outputs *= 1000
    elif prop == "vap":
        y_label = "Experimental Vapor Pressure [log(mmHg)]"
    else:
